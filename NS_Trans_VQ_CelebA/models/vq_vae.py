...
import math
from vector_quantize_pytorch import VectorQuantize as VectorQuantize_from_lucidrains
from vector_quantize_pytorch import SimVQ, CodeAsBatchTokenTransformer, CodebookTransformer
#%%
class VectorQuantizer(nn.Module):
    ...
    def __init__(self,
                ...
                mem_flag: bool = False,
                embedding_loss_weight = 0.01): # default p = 0
    ...
    def forward(self, latents: Tensor, epoch: int = None, max_epoch: int = None, status: str = 'Train') -> Tensor:
        ...
        if self.mem_flag and status == 'Train' and epoch is not None and max_epoch is not None:
            ...
            tau = self.get_decay_p(
                    ...
                    peak_p=1,
                    **{'decay_rate': 0.50} )
            soft_weights = self.embedding_loss_weight * self.K * self.mem_sample(dist, tau=tau).detach() #[BWH,k]
            ...
            # diff = flat_latents.unsqueeze(1).detach() - self.embedding.weight.unsqueeze(0)  # [BHW, K, D]
            # squared_error = diff.pow(2)  # [BHW, K, D]
            # mse_per_embedding = squared_error.mean(dim=-1)  # [BHW, K]

            # # Weighted embedding loss
            # embedding_loss = (combined_weights  * mse_per_embedding).sum() / flat_latents.shape[0]

            # --- memory-safe embedding loss (blocks over samples AND codebook) ---
            embedding_loss = latents.new_tensor(0.)

            # tune these two for your GPU
            m_block = 1024          # how many latents per block
            k_block = 8912         # how many codes per block

            num_latents = flat_latents.shape[0]
            D = self.D

            with torch.no_grad():
                # precompute ||x||^2 once per sample block
                x2_full = None  # computed inside the loop

            # iterate over sample blocks
            for m_start in range(0, num_latents, m_block):
                m_end = min(m_start + m_block, num_latents)
                x = flat_latents[m_start:m_end].detach()              # [m, D], no grad to inputs
                x2 = (x * x).sum(dim=1, keepdim=True)                 # [m, 1]

                # iterate over codebook blocks
                for k_start in range(0, self.K, k_block):
                    k_end = min(k_start + k_block, self.K)
                    E = self.embedding.weight[k_start:k_end]          # [k, D]
                    e2 = (E * E).sum(dim=1)                           # [k]

                    # distances^2 = ||x||^2 + ||e||^2 - 2 x·e
                    # shape: [m, k]
                    cross = x @ E.t()
                    dist2 = x2 + e2.unsqueeze(0) - 2.0 * cross

                    # mean over D to match your previous "mse_per_embedding"
                    mse_block = dist2 / D                              # [m, k]

                    # take matching weight slice and accumulate
                    w = combined_weights[m_start:m_end, k_start:k_end] # [m, k]
                    embedding_loss = embedding_loss + (w * mse_block).sum()

            embedding_loss = embedding_loss / num_latents
...
class VQ_new(nn.Module):
    ...
    def forward(self, latents: Tensor, epoch: int = None, max_epoch: int = None, status: str = 'Train') -> Tensor:
        ...
        if status == 'Train' and epoch is not None and max_epoch is not None:
            ...
            one_hot_winner = encoding_one_hot          # [BHW, K]
            # Combine: winner → 1, others → soft weight
            combined_weights = soft_weights * (1 - one_hot_winner)
            combined_weights = torch.clamp(combined_weights, min=0.0, max=1.0)
        else:
            combined_weights = torch.zeros(encoding_one_hot.shape, device=device)
        embedding_loss = F.mse_loss(quantized_latents, latents.detach()) # [B x H x W x D] vs [B x H x W x D]
        ...
        alpha_q = 2 * self.alpha / (latents_shape[0] * latents_shape[1] * latents_shape[2] * latents_shape[3])
        addition = torch.matmul(combined_weights, self.embedding.weight).view(latents_shape)
        quantized_latents = latents + alpha_q * quantized_latents + ((1-alpha_q)*quantized_latents - latents).detach() + addition - addition.detach()     # method 3, corrected formula
...
class VQVAE(BaseVAE):
    ...
    def __init__(self,
        ...
        vq_mod = vq_parameters.get("vq_mod", 'vq')
        decay = vq_parameters.get("decay", 0.8)
        ...
        if self.vq_flag:
            if self.vq_mod == 'vq':
            ...
            elif self.vq_mod == 'ema':
                self.vq_layer = VectorQuantize_from_lucidrains(
                    dim = embedding_dim,
            ...
            elif self.vq_mod == 'vq_new':
                self.vq_layer = VQ_new(
                    num_embeddings = num_embeddings,
                    embedding_dim = embedding_dim,
                    beta = beta,
                    alpha_q = alpha,
                ...
                self.vq_layer = SimVQ(
                                    dim = embedding_dim,
                                    codebook_transform = CodebookTransformer(
                                        frozen_codebook_dim=embedding_dim,
                                        out_dim=embedding_dim,
                                        codebook_size=num_embeddings,
                                        depth=1,            # increase for more capacity
                                        model_dim=256,
                                        n_heads=1,
                                        mlp_ratio=2.0,
                                        dropout=0.0,
    ...
    def forward(self, input: Tensor, epoch: int = None, max_epoch: int = None, status: str = 'Train', **kwargs) -> List[Tensor]:
        ...
        if self.vq_mod == 'vq' or self.vq_mod == 'vq_new':
            quantized_inputs, vq_loss, encoding_inds = self.vq_layer(encoding, epoch, max_epoch, status)
        else:
            quantized_inputs, encoding_inds, vq_loss  = self.vq_layer(encoding)
