...
from ldm.modules.vector_quantize_pytorch import rotate_to
# robust "mapping-like" detection for YAML/OmegaConf configs
try:
...
try:
    ...
    _MAPPING_LIKE = (Mapping, DictConfig)
except Exception:
    _MAPPING_LIKE = (Mapping, dict)        # fallback if OmegaConf not installed
...
class SimVQ(Module):
    def __init__(
        ...
        if codebook_transform is None:
            # Case 1: not provided -> use a simple linear projection
            codebook_transform = nn.Linear(frozen_codebook_dim, dim, bias=False)

        elif isinstance(codebook_transform, nn.Module):
            # Case 2: already an instantiated nn.Module -> use it as-is
            pass

        elif isinstance(codebook_transform, _MAPPING_LIKE):
            # Case 3: provided via YAML/Config -> instantiate from config
            # Expecting something like:
            # codebook_transform:
            #   target: mypkg.mymod.MyProj
            #   params: { in_features: ..., out_features: ..., ... }
            codebook_transform = instantiate_from_config(codebook_transform)

        else:
            raise TypeError(
                "codebook_transform must be None, an nn.Module, or a config Mapping "
                "(e.g. a dict with keys {'target', 'params'})."
            )
    ...
    def forward(
        ...
        perplexity = None
        min_encodings = None
        return quantized,  commit_loss * self.commitment_weight, (perplexity, min_encodings, indices)
