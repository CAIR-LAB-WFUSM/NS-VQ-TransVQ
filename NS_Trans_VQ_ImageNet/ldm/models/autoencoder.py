...
class VQModel(pl.LightningModule):
    def __init__(self,
        ...
        self._raw_configs = {
            ...
            "scheduler_config": self.scheduler_config,  # or the arg you pass in
            "n_embed": n_embed,
            "embed_dim": embed_dim,
        }
    ...
    def on_fit_start(self):
        ...
        log_dir = getattr(self.trainer, "log_dir", None) or \
                getattr(self.logger, "log_dir", None) or \
                self.trainer.default_root_dir
        Path(log_dir).mkdir(parents=True, exist_ok=True)

        # save raw (unresolved)
        cfg = OmegaConf.create(self._raw_configs)
        with open(os.path.join(log_dir, "config_raw.yaml"), "w", encoding="utf-8") as f:
            yaml.safe_dump(OmegaConf.to_container(cfg, resolve=False), f, allow_unicode=True, sort_keys=False)
